...
-- URL: https://atlas-demo.ohdsi.org/#/cohortdefinition/1779686
...
( 
  select concept_id from CONCEPT where concept_id in (40481089,440417,254662,318775,444247,36713113,35615055)
...
LEFT JOIN
(
  select concept_id from CONCEPT where concept_id in (435616,435887,196715,4062269,442055,433832,435026,440477,318137,199837,438820,4235812,195294,4187790)
UNION  select c.concept_id
  from CONCEPT c
  join CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
